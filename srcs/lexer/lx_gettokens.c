...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/06/16 15:54:46 by acorbeau          #+#    #+#             */
/*   Updated: 2017/06/16 15:56:51 by acorbeau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_lexer		*lx_gettokens(t_lexer *lx)
{
	...
	while (*lx->ptr)
	{
		...
		if (lx->stat == LXS_PARSED)
			return (lx);
		else if (lx->sesc || lx->desc ||
				stat_match(lx->stat, get_stat(lx->ptr)))
			put_charwd(lx);
		else
			break ;
		lx->ptr++;
